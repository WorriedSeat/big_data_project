...
BEELINE_CSV="beeline -u jdbc:hive2://hadoop-03.uni.innopolis.ru:10001 -n team14 -p $hive_password --hiveconf hive.resultset.use.unique.column.names=false --outputformat=csv2 --silent=true"
...
$BEELINE_CSV -e "USE team14_projectdb; SELECT * FROM q1_results;" > output/q1.csv
echo "Done. See output/q1.csv"

echo "=== Running Q2: Monthly flight volume and delays ==="
$BEELINE -f sql/q2.hql 2>&1 | tee output/q2_hive.txt
$BEELINE_CSV -e "USE team14_projectdb; SELECT * FROM q2_results;" > output/q2.csv
echo "Done. See output/q2.csv"

echo "=== Running Q3: Top 10 busiest routes ==="
$BEELINE -f sql/q3.hql 2>&1 | tee output/q3_hive.txt
$BEELINE_CSV -e "USE team14_projectdb; SELECT * FROM q3_results;" > output/q3.csv
echo "Done. See output/q3.csv"

echo "=== Running Q4: Cancellation rate by airline ==="
$BEELINE -f sql/q4.hql 2>&1 | tee output/q4_hive.txt
$BEELINE_CSV -e "USE team14_projectdb; SELECT * FROM q4_results;" > output/q4.csv
echo "Done. See output/q4.csv"

echo "=== Running Q5: Average delay by cause ==="
$BEELINE -f sql/q5.hql 2>&1 | tee output/q5_hive.txt
$BEELINE_CSV -e "USE team14_projectdb; SELECT * FROM q5_results;" > output/q5.csv
echo "Done. See output/q5.csv"
